...
export function EventCard({ event, onViewDetails, isLast }: EventCardProps) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="relative flex items-center gap-6 p-6 cursor-pointer" onClick={onViewDetails}>
        <motion.div
          className="relative flex-shrink-0"
          whileHover={{ scale: 1.05 }}
          transition={{ duration: 0.3 }}
        >
          <div className="absolute inset-0 bg-gradient-to-br from-[#a855f7]/20 to-[#ec4899]/20 rounded-lg blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          <img
            src={event.thumbnail}
            alt={event.title}
